...
export const authAPI = {
  async login(data: LoginData) {
    if (isMockMode) {
      ...
      // Find user in mock test users
      const user = mockTestUsers.find(u => u.username === data.username && u.password === data.password);
  ...
  async getProfile(): Promise<ProfileData> {
    if (isMockMode) {
      ...
      // Get the actual current user from localStorage
      const currentUser = this.getCurrentUser();
      
      // Return profile data based on the actual logged-in user
      return {
        _id: currentUser?._id || '1',
        fullName: currentUser?.username || 'Admin User',
        email: currentUser?.email || 'admin@doctorcare.com',
        phone: '+1 (555) 123-4567',
        location: 'New York, NY',
        dateOfBirth: '1985-03-15',
        gender: 'Female', // Sample gender for mock profile
        department: 'Administration',
        position: currentUser?.role === 'admin' ? 'System Administrator' : 'Doctor',
        employeeId: currentUser?.role === 'admin' ? 'ADM-2024-001' : 'DOC-2024-001',
        joinDate: '2020-01-15',
        profileImage: currentUser?.role === 'admin' 
          ? 'https://randomuser.me/api/portraits/men/34.jpg' 
          : 'https://randomuser.me/api/portraits/women/44.jpg',
        permissions: currentUser?.role === 'admin' 
          ? ['User Management', 'Appointment Management', 'System Settings', 'Reports & Analytics']
          : ['Patient Management', 'Appointment Management', 'Medical Records'],
        stats: {
          patients: currentUser?.role === 'admin' ? 156 : 89,
          appointments: currentUser?.role === 'admin' ? 89 : 156,
          rating: 4.9
        }
      };
    }
  ...
  async updateProfile(data: UpdateProfileData): Promise<ProfileData> {
    if (isMockMode) {
      ...
      // In mock mode, return the updated data based on current user
      return {
        _id: currentUser?._id || '1',
        fullName: data.fullName || currentUser?.username || 'Admin User',
        email: data.email || currentUser?.email || 'admin@doctorcare.com',
        phone: data.phone || '+1 (555) 123-4567',
        location: data.location || 'New York, NY',
        dateOfBirth: data.dateOfBirth || '1985-03-15',
        gender: 'Female', // Sample gender for mock profile
        department: data.department || 'Administration',
        position: data.position || (currentUser?.role === 'admin' ? 'System Administrator' : 'Doctor'),
        employeeId: currentUser?.role === 'admin' ? 'ADM-2024-001' : 'DOC-2024-001',
        joinDate: '2020-01-15',
        profileImage: currentUser?.role === 'admin' 
          ? 'https://randomuser.me/api/portraits/men/34.jpg' 
  ...
  async uploadProfileImage(file: File): Promise<{ imageUrl: string }> {
    if (isMockMode) {
      ...
      // In mock mode, return a mock image URL
      return {
...
export const patientsAPI = {
  ...
  async getPatientById(id: string) {
    if (isMockMode) {
      ...
      if (!patient) {
        console.warn(`Patient with ID ${id} not found, returning first available patient`);
        // Return the first patient as a fallback instead of throwing an error
  ...
  async getPatientMedicalSuggestions(id: string) {
    if (isMockMode) {
      await simulateApiDelay();
      simulateApiError();
      return [];
